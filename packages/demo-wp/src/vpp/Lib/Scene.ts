...
class PowerRouterScene {
  ...
  load() {
    console.log('load power router scene');
    if (this.camera) {
      return;
    }
    if (!this.container) {
    ...
    circle.rotation.copy(new THREE.Euler(Math.PI / 2, 0, 0));
    this.scene.add(circle);

    console.log('load power router scene done!');
  }
...
export default PowerRouterScene;
